...
lang_dict = {
    ...
    "codet5": ["python", "java", "javascript", "php", "ruby", "c", "c++", "c-sharp", "go"]
}
...
if __name__ == "__main__":
    ...
    if args.doCSLRD and args.rank > len(languages):
        parser.error("--rank for CSLRD cannot be greater than model's known languages (" + str(len(languages)) + ")")
    ...
    if args.doCentering or args.doLRD:
        for lang in languages:
            ...
            for i in range(0,args.number):
                tensor = torch.load(embed_path + str(i) + ".txt.pt", weights_only=True, map_location=torch.device(sDevice))
                tensors.append(tensor)
                
            if args.doCentering:
                mean_embedding = torch.unsqueeze(torch.mean(torch.stack(tensors), 0), 0)
                torch.save(mean_embedding, os.path.join(model_path, model_type + "_mean_" + lang + ".pt"))
            
            if args.doLRD:
                _, _, Vh = torch.linalg.svd(torch.stack(tensors))
                P = torch.matmul(Vh[:,:r], T(Vh[:,:r]))
                torch.save(P, os.path.join(model_path, model_type + "_LRD" + str(r) + "_" + lang + ".pt"))
    ...
    if args.doCSLRD:
        ...
        for lang in languages:
            ...
            try:
                mean_embedding = torch.load(file_path, weights_only=True, map_location=torch.device(sDevice))[0]
            except FileNotFoundError:
                print("FileNotFoundError: No mean language embedding found at " + file_path + ". Run with doCentering as True to generate these files.")
                exit()
        ...
        torch.save(MsMsT, os.path.join(model_path, model_type + "_CSLRD" + str(r) + ".pt"))
